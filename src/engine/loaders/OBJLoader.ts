...
type _Object = {
  ...
  materials: _Material[];
  smooth: boolean;
  startMaterial: (name: string, libraries: string[]) => _Material;
  currentMaterial: () => _Material | undefined;
  _finalize: (end: boolean) => _Material | undefined;
};
...
class ParserState {
  ...
  startObject(name: string, fromDeclaration?: boolean) {
    ...
    this.object = {
      ...
      startMaterial: function (name: string, libraries: string[]): _Material {
        ...
        const material: _Material = {
          ...
          clone: function (index: number): _Material {
            const cloned = {
  ...
  addFaceNormal(a: number, b: number, c: number) {
    ...
    vec3.set(_vA, src[a + 0], src[a + 1], src[a + 2]);
    vec3.set(_vB, src[b + 0], src[b + 1], src[b + 2]);
    vec3.set(_vC, src[c + 0], src[c + 1], src[c + 2]);

    vec3.sub(_cb, _vC, _vB);
    vec3.sub(_ab, _vA, _vB);
    vec3.cross(_cb, _cb, _ab);
    vec3.normalize(_cb, _cb);

    dst.push(_cb[0], _cb[1], _cb[2]);
    dst.push(_cb[0], _cb[1], _cb[2]);
    dst.push(_cb[0], _cb[1], _cb[2]);
  }
  ...
  addColor(a: number, b?: number, c?: number) {
    ...
    if (b !== undefined && src[b] !== undefined)
      dst.push(src[b + 0]!, src[b + 1]!, src[b + 2]!);
    if (c !== undefined && src[c] !== undefined)
      dst.push(src[c + 0]!, src[c + 1]!, src[c + 2]!);
  }
...
class OBJLoader {
  materials: MaterialCreator | null = null;

  setMaterials(materials: MaterialCreator) {
  ...
  parse(text: string): Group {
    ...
    if (hasPrimitives === true) {
      for (let i = 0, l = state.objects.length; i < l; i++) {
        ...
        buffergeometry.setPositions(geometry.vertices, 3);

        if (geometry.normals.length > 0) {
          buffergeometry.setNormals(geometry.normals, 3);
        }

        if (geometry.colors.length > 0) {
          hasVertexColors = true;
          buffergeometry.setColors(geometry.colors, 3);
        }

        if (geometry.hasUVIndices === true) {
          buffergeometry.setUVs(geometry.uvs, 2);
        }
        ...
        for (let mi = 0, miLen = materials.length; mi < miLen; mi++) {
          ...
          if (this.materials !== null) {
            ...
            if (isLine && material && !(material instanceof LineMaterial)) {
              ...
              const materialPoints = new PointsMaterial(10, false);
          ...
          if (material === undefined) {
            if (isLine) {
              ...
              material = new PointsMaterial(1, false);
            } else {
      ...
      if (state.vertices.length > 0) {
        const material = new PointsMaterial(1, false);

        const buffergeometry = new Geometry();

        buffergeometry.setPositions(state.vertices, 3);

        if (state.colors.length > 0 && state.colors[0] !== undefined) {
          buffergeometry.setColors(state.colors as number[], 3);
